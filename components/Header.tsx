        ...
        <div className="container flex items-center justify-between">
            ...
            <nav className={`lg:hidden absolute top-full left-0 w-full bg-white/60 backdrop-blur-2xl border-y border-gray-200 flex items-center justify-center -z-30 -translate-y-[170%] transition-transform ${isOpen ? "translate-y-0" : ""}`}>
                <ul className="py-6 text-center">
                    {navItems.map((item) => (
                        <li key={item.id}>
                            <Link href={item.href} className={`uppercase p-1.5 inline-block hover:text-primary-600 transition-color font-medium ${pathname === item.href ? "text-primary-600" : ""}`} onClick={handleClick}>{item.label}</Link>
                        </li> 
                    ))}
                </ul>
            </nav>
            ...
            {/* Menu Btn */}
            <button className="lg:hidden border border-gray-300 size-11 flex items-center justify-center rounded-lg hover:bg-gray-100 transition-colors focus:bg-gray-150" onClick={handleClick}>
